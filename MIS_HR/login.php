    ...
    if (empty($username_err) && empty($password_err)) {
        ...
        if ($stmt->fetch()) {
            if (password_verify($password, $hashed_password)) {
                // Close the SELECT statement before executing the UPDATE query
                $stmt->close();

                // Generate a unique session token
                $session_token = bin2hex(random_bytes(32));

                // Update the session token and set status to 1 in the database
                ...
                $_SESSION["id"] = $id;
                $_SESSION["username"] = $db_username; // Set the username
        ...
        // Close the SELECT statement if not already closed
        $stmt->close();
    }
